/**
 * TODO #0819 : 토큰 필터 구현  
 * - 종 류 ( 아래 둘중 하나를 상속 받아서 구현 후 등록하여 사용)
 * 	- OncePerRequestFilter : 동일한 요청에 대하여 한번만 필터링하는 용도
 * 	- GenericFilter : 일반적인 필터
 * 
 * - 토큰 필터에 DI 처리는 생성자 방식을 사용한다.
 * - TokenAuthenticationFileter 객체가 만들어질때 주입 => 생성자 파라미터로 세팅
 */
...
//	상수 정의
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		//	  정적 데이터, h2-console, /user, 기타 필요한 것들 추가
...
//		if(list.length>=2) { //for 활용 가능, 로직 변경 가능함
		...
		// 웹앱 , SPA등 서비스에서 진입했을때 => restapi 방식으로 진입했을떄
		...
		if(tokenProvider.isValidToken(accessToken)) { // 유효함 => 정상 처리 => 인증정보 세팅하여 해당 페이지로 이동
			...
			// 인증 정보 생성후 -> 인증정보 관리 객체(시큐리티 컨텍스트 홀더)에 추가
			// 원하는 페이지 이동
			filterChain.doFilter(request, response);
		}
		else { // 유효하지 않다 => 리프레시 토큰 활용하여 재발급=> 이후 진행
			System.out.println("유효하지 않은 토큰");
//			리프레시 토큰 검증 진행 ( 토큰 자체는 문제 없는지, 디비에 저장 되어있는지)
...
//			
			...
			// user id => SnsUser 객체 획득
			//SnsUser user = userService.findById(rToken.getUserId());
			// SnsUser => accessToken
			//accessToken  = this.tokenProvider.generateToken(user, Duration.ofDays(7));
			// 순환 참조 오류(DI 남발) 발생시 다른 컨트롤러에서 해결하게 조정 => http로 던져서 컨트롤러에서 해결
			// 요청에대한 처리 권한을 다른쪽(컨트롤러)으로 보내서 해당 컨트롤러에서 해결 
