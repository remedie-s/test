...
//	}
	...
	// <문제점>
	// 본글에 참조키로 연결된 리뷰 데이터가 존재할 경우 응답값이 무한 반복(무한참조)이됨-> 본문따로, 리뷰 따로(페이징)
	// 본글만 있으면 문제 없음
	// 노출되면 안되는 값들도 모두 전달된다.-> 무시전략 (설정 JSON 변경시 노출안되게 처리)
	...
	//@Tag 같은 비지니스 계열 API간 그룹화 처리가능
	@Tag(name="포스트 게시판", description= "ResponseEntity를 통해 응답처리")
	@Parameters({
		@Parameter(name = "id", description = "포스트 글의 고유 번호값", example = "1")
	})
	
	// 통신 성공후, 응답 데이터에 대한 의미(도)를 코드로 전달할 때, 그 값을 체크하는 용도
	// 통신 프로토콜표 = 전문 = TR
	@ApiResponses(value= {
	...
	// 그룹내 api별 구분용도 사용
	@Operation(summary = "포스트 정보 조회(헤더 조작가능)", description = "포스트 내용 조회후 응답시 헤더를 커스텀하게 조정할 수 있다.")
	// ResponseEntity는 사용하면 -> 응답 헤더 조정 가능함
